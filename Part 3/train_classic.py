...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(
            model.state_dict(),
            os.path.join(output_dir, f"classic_checkpoint_epoch{epoch + 1}.pth"),
        )
    # Save best model
    if val_accs[-1] > best_val_acc:
        best_val_acc = val_accs[-1]
        best_model_path = os.path.join(output_dir, f"classic_best_model.pth")
...
plt.legend()
plt.title("Accuracy")
plt.savefig(os.path.join(output_dir, "classic_training_curves.png"))
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "classic_confusion_matrix.png"))
plt.show()
